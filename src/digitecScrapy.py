...
import zlib
import os
from src.model.articleDetail import Article, DeliveryOption
...
class DigitecScrapy():
    ...
    def get_article_details(self, article_number: int, print_out: bool = False, safe_zio: bool = False, safe_json: bool = False, path: str = 'data') -> Article:
        ...
        if response.status_code == 200:
            if 'errors' in response.json()[0] and response.json()[0]['errors'][0]['extensions']['code'] == "NOT_FOUND":
                raise NotFoundException("Article not found")
            article = Article(response.json())
            if safe_zio: self.__safe_zio(response.json(), path, article_number)
            if safe_json: self.__safe_json(response.json(), path, article_number)
            if print_out: self.__print_out(article)
            return article
        elif response.status_code == 403:
            raise HitRateLimitException("Hit rate limit")
        elif response.status_code == 404:
            raise NotFoundException("Article not found")
        else:
            raise Exception("Unspecified Error")

    def __call_detail_api(self, article_number: str):
    ...
    def __print_out(self, article: Article):
    ...
    def _get_column_size(self, article: Article):
        first_column = 28
        second_column = 40
